...
const localStoragePrefix = 'entities-input'
...
onMounted(() => {
    ...
    const entityTypeId = props.context.typeName === 'all' ? 0 : getValueOfLookupTpeByLabel('entity_types', props.context.typeName, t)
    if (entityTypeId != null) {
        apiClient.entitiesInputList({ entityTypeId }).then(
            (res: any) => {
